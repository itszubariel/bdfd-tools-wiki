  ...
  if (!isNaN(inputValue) && inputValue !== "") {
    ...
    if (inputValue === "") {
      ...
      nonNumericIndex =
        nonNumericIndex === -1
          ? inputValue.length
          : nonNumericIndex + functionName.length + 3;
...
function sqrtPlayground(inputValue) {
  handlePlaygroundInput(inputValue, "$sqrt", (value) =>
    Math.round(Math.sqrt(value)),
  );
}
...
function toLowercasePlayground(inputValue) {
  const playOutput = document.getElementById("play-output");
  playOutput.innerHTML = preserveLineBreaks(
    limitLines(inputValue.toLowerCase(), 20),
  );
  editInputHeight();
}
...
function toUppercasePlayground(inputValue) {
  const playOutput = document.getElementById("play-output");
  playOutput.innerHTML = preserveLineBreaks(
    limitLines(inputValue.toUpperCase(), 20),
  );
  editInputHeight();
}
...
function updateCloseAllButton() {
  ...
  const errorMessageElement = document.getElementById("error-message");
  const errorCount =
    errorMessageElement.children.length - (closeAllButton ? 1 : 0);
...
function getRelativeTime(date) {
  ...
  for (const interval in intervals) {
    ...
    if (quantity >= 1) {
      return (
        prefix + quantity + " " + interval + (quantity > 1 ? "s" : "") + suffix
      );
    }
  }
...
function typeScript() {
  ...
  if (nameLabel) nameLabel.textContent = "Name" + commandType;
}
...
function callButtonChange(buttonName, status) {
  ...
  const extraGradient =
    "linear-gradient(to right, rgb(255 255 255 / 40%), rgb(0 0 0 / 50%))";
...
const colorNames = {
  ...
  empire: "#00549e",
  empireyellow: "#f7d000",
  enamelsapphire: "#126180",
  enamelturquoise: "#00a78e",
  english: "#b48395",
  englishgreen: "#1b4d3e",
...
function formatDateInTimezone(date, timezone) {
  ...
  try {
    ...
    const minute = parts
      ...
      .value.padStart(2, "0");
    const second =
      parts.find((p) => p.type === "second")?.value.padStart(2, "0") || "00";
...
function getCurrentTimeInTimezone() {
  ...
  try {
    ...
    const minute = dateParts
      ...
      .value.padStart(2, "0");
    const second =
      dateParts.find((p) => p.type === "second")?.value.padStart(2, "0") ||
      "00";
...
function getUnixTimeForNewYear(year, timezone) {
  try {
    const dateString = `${year}-01-01T00:00:00`;
    ...
    if (timezone) {
      ...
      const adjustedDate = new Date(formatted);
...
function getUnixTimeForNextDay(timezone) {
  try {
    const now = new Date();
    ...
    if (timezone) {
      ...
      const formatted = formatter.format(tomorrow);
      const adjustedDate = new Date(formatted);
      return Math.floor(adjustedDate.getTime() / 1000);
    } else {
...
function formatTimeDifference(timestamp) {
  ...
  if (absDiff < 60) {
    return diff >= 0
      ? `in ${absDiff} second${absDiff !== 1 ? "s" : ""}`
      : `${absDiff} second${absDiff !== 1 ? "s" : ""} ago`;
  }
...
function createQuickButtons() {
  ...
  const times = [
    ...
    { id: "newDay", label: "Next day", type: "newDay" },
  ...
  times.forEach((time) => {
    ...
    button.onclick = function () {
      ...
      let unixTime;
      let baseTime = Math.floor(Date.now() / 1000) + relativeOffset;

      if (time.type === "now") {
        unixTime = Math.floor(Date.now() / 1000);
        relativeOffset = 0;
        for (const key in clickCounters) {
          if (key !== "newDay" && key !== "newYear") {
            clickCounters[key] = 0;
          }
        }
      } else if (time.type === "relative") {
        relativeOffset += time.seconds;
        unixTime = Math.floor(Date.now() / 1000) + relativeOffset;
      } else if (time.type === "newYear") {
        const now = new Date();
        const nextYear = now.getFullYear() + multiplier;
        button.textContent = `New Year ${nextYear}`;
        unixTime = getUnixTimeForNewYear(nextYear, currentTimezone);
      } else if (time.type === "newDay") {
        unixTime =
          getUnixTimeForNextDay(currentTimezone) + 86400 * (multiplier - 1);
      }
...
function copyPermissionValue() {
  ...
  if (totalValueElement && totalValueElement.textContent.trim()) {
    copyToClipboard(
      totalValueElement.textContent.trim(),
      "permissionCopySuccess",
    );
  }
}
...
function hexInputChange() {
  ...
  if (val === "") {
    validColorDisplay.innerHTML =
      '<div class="valid-color" style="background:#000000" onclick="copyToClipboard(\'#000000\', \'colorCopySuccess\')">#000000</div><span class="copy-success" id="colorCopySuccess">Copied!</span>';
