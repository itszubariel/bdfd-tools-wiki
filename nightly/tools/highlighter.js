(function () {
  ...
  fetch("functions.json")
    ...
    .then(function (data) {
      ...
      console.log(
        "Loaded " + allFunctions.length + " functions from functions.json",
      );
    })
  ...
  window.addEventListener("load", function () {
    ...
    function renderFunctionList() {
      ...
      allFunctions
        .filter(function (fn) {
          return fn.toLowerCase().includes(query) && !theme[fn];
        })
        .forEach(function (fn) {
    ...
    function openEditModal(fn) {
      ...
      styleButtons.forEach(function (btn) {
        btn.classList.toggle(
          "active",
          parseInt(btn.dataset.style) === currentEditStyle,
        );
      });
    ...
    function updateModalPreview() {
      ...
      modalPreview.style.fontWeight =
        currentEditStyle === 1 || currentEditStyle === 3 ? "bold" : "normal";
      modalPreview.style.fontStyle =
        currentEditStyle === 2 || currentEditStyle === 3 ? "italic" : "normal";
    ...
    applyImport.onclick = function () {
      ...
      for (const key in importedTheme) {
        ...
        if (
          baseKeyMap[key] &&
          importedTheme[key] &&
          typeof importedTheme[key].color === "number" &&
          typeof importedTheme[key].style === "number"
        ) {
    ...
    copyOutputBtn.onclick = function () {
      if (
        !output.textContent ||
        output.textContent === "Your highlighted code will appear here..."
      ) {
